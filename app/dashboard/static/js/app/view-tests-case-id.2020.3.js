...
require([
    ...
    function updateGroupDetails(results) {
        ...
        if (!results.modules_dir || results.modules_dir == 'None') {
            modulesNode = html.nonavail();
        } else {
            var modulesPath;

            modulesPath = translatedURI[1] + "/" + results.modules_dir;
    ...
    function getGroup(results) {
        console.log("group_id: " + results.test_group_id.$oid)
    ...
    function getCaseDone(response) {
        getGroup(response.result[0]);
        updateCaseDetails(response.result[0]);
    }
