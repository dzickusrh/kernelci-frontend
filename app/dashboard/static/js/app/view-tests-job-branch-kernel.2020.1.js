...
require([
    ...
    'tables/test',
    'charts/passpie',
], function($, init, format, urls, html, error, request, table, ttest, chart) {
    'use strict';
    var gFileServer;
    var gJob;
    var gBranch;
    ...
    function updateDetails(results) {
        ...
        describeNode.appendChild(document.createTextNode(gKernel));
        describeNode.insertAdjacentHTML('beforeend', '&nbsp;&mdash;&nbsp;');
        describeNode.appendChild(buildsLink);

        jobNode = document.createElement('a')
        jobNode.appendChild(document.createTextNode(results.git_url))
        jobNode.href = results.git_url
        jobNode.title = "Git URL" /* ToDo: link to commit when possible */
        ...
        html.replaceContent(
            document.getElementById('git-url'), jobNode);
        html.replaceContent(
    ...
    function updatePlansTable(results) {
        ...
        gPlansTable
            ...
            .columns(columns)
            .order([0, 'asc'])
            .rowURL('/test/%(job)s/branch/%(git_branch)s/kernel/%(kernel)s/plan/%(name)s/')
